...
const AdminBotPanel: React.FC = () => {
  ...
  useEffect(() => {
    loadBotStatus();
    loadUrlTargets();
    const interval = setInterval(loadBotStatus, 10000); // Refresh every 10 seconds
    return () => clearInterval(interval);
  }, []);

  const loadBotStatus = async () => {
    try {
      const response = await fetch('/api/bot-queue-status');
